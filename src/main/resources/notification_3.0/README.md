...
- **notification:item-type**: The type of ORCID item represented by this ```item``` element. Chosen from the following: ```education```, ```employment```, ```qualification```, ```invited-position```, ```distinction```, ```membership```, ```service```, ```funding```, ```work```, or ```peer-review```.
